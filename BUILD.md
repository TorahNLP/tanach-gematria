...
_build_connection()         ← @st.cache_resource; builds in-memory SQLite
        ...
        ├─ _accent_phrases(TIPCHA|ATNACH)        ← TiphchaPhrase rows (sub-half phrase units)
        ├─ _accent_phrases(ZAKEF|TIPCHA|ATNACH)  ← ZakefPhrase rows (finest cantillation units)
...
Cantillated text is threaded to **every row type** — Verse, TiphchaPhrase, ZakefPhrase, FirstHalf, SecondHalf, Word, Petucha, Setuma. `_tokenize_raw_words` drops letter-less tokens (paseq ׀, sof-pasuq) before index-aligning with `tokenize_words`, preventing off-by-one misalignment on verses containing paseq.
...
| `boundary_type` | Word / ZakefPhrase / TiphchaPhrase / FirstHalf / SecondHalf / Verse / Petucha / Setuma / Perek / Parsha |
...
- **Nikud ciphers on aggregate rows** — Perek and Parsha aggregate rows concatenate consonants only, so `HaNekudot`/`MiluiNekudot` = 0 for those rows. All other row types (Verse, TiphchaPhrase, ZakefPhrase, FirstHalf, SecondHalf, Word, Petucha, Setuma) carry real nikud values.
- **TiphchaPhrase / ZakefPhrase** — generated by `_accent_phrases()`. TiphchaPhrase splits on TIPCHA (U+0596) + ATNACH; ZakefPhrase additionally splits on ZAKEF_KATON (U+0594). Every phrase ends with (and includes) the word bearing the split accent. A trailing remainder (words after the last split accent) is also stored. ~3 TiphchaPhrase and ~5 ZakefPhrase rows per verse.
- **`span_search()`** — Tab 1 only. Queries all Word rows for a cipher, computes numpy prefix sums per verse, and returns every 2-to-N word span matching the target value. No schema change; purely computational at query time.
